...
Rendered via sharp `text` API with `fontfile` paths plus `fonts.conf` (`FONTCONFIG_FILE`) in `glance-render.ts`. Lambda packages must include this directory (`scripts/copy-api-lambda-assets.mjs`).
