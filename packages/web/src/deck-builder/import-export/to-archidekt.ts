import { applyFormalSwapsToCards, type DeckDocument } from '@rayenz-hub/shared';
...
export function buildArchidektImportText(doc: DeckDocument): string {
  ...
  const catMeta = new Map(doc.categories.map((c) => [c.name, c]));
  const byCat: Record<string, typeof cards> = {};
  for (const card of cards) {
    const key = card.primaryCategory || 'Main';
  ...
  for (const cat of order) {
    ...
    lines.push(
      categoryHeader(cat, meta ? meta.includedInDeck !== false : true, meta ? meta.includedInPrice !== false : true),
    );
