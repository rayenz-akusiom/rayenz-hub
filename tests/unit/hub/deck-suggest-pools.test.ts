...
describe('Data.indexSetPool', () => {
  ...
  it('is idempotent when called twice', () => {
    const scope = Data.indexSetPool({ cards: [{ name: 'Sol Ring' }] })!;
    const again = Data.indexSetPool(scope)!;
    expect(again).toBe(scope);
    expect(again.cardsByName!['sol ring']).toHaveLength(1);
  });
});

describe('Data.buildDeckRuleContext', () => {
  ...
  it('returns the same context on repeat calls', () => {
    const deck = sampleDeck();
    const first = Data.buildDeckRuleContext(deck);
    const second = Data.buildDeckRuleContext(deck);
...
describe('DeckSuggest precomputed rule pools', () => {
  it('findInSetPool uses cardsByName index', () => {
    ...
    const card = RuleGuards.findInSetPool('Take Up the Shield', scope);
  ...
  it('runRulesForDeck precomputes pools before running rules', () => {
    const deck = sampleDeck();
    const setScope = Data.indexSetPool({
      primaryCode: 'MSH',
      codes: ['MSH'],
      cards: [{ name: 'New Card', set_code: 'MSH', collector_number: '1' }],
    })!;
    runRulesForDeck(deck, setScope, {});
