/**
 * Scryfall + Wizards Collecting helpers (ported from mtg-deck-set-updates scripts).
 */

const SCRYFALL_API = 'https://api.scryfall.com';
const WIZARDS_BASE = 'https://magic.wizards.com/en/news/feature';
const USER_AGENT = 'rayenz-hub-mcp/1.0';
...
async function resolveViaWizards(
  ...
  for (const candidate of candidates) {
    try {
      console.error(`Fetching Collecting article: ${candidate}`);
      ...
      console.error(`No set codes found at ${candidate}`);
    } catch (e) {
      const status = (e as { status?: number }).status;
      console.error(status ? `HTTP ${status} for ${candidate}` : `Failed to fetch ${candidate}: ${e}`);
    }
  }
  return null;
}
...
async function fetchAllSets(): Promise<Array<Record<string, unknown>>> {
  ...
  while (url) {
    const data = (await fetchJson(url)) as { data?: Array<Record<string, unknown>>; next_page?: string };
    sets.push(...(data.data || []));
...
async function enrichSets(
  ...
  Array<ScryfallSetMeta & { label: string | null }>
> {
  const sets: Array<ScryfallSetMeta & { label: string | null }> = [];
  for (const code of codes) {
    const meta = await fetchSetMetadata(code);
    if (!meta) {
      console.error(`Warning: ${code} not found on Scryfall, skipping.`);
      continue;
    }
...
export function analyzableOracleText(card: Record<string, unknown>): string {
  ...
  // Prefer faces when they add text not already present (adventure backs, etc.)
...
export async function fetchSetCards(
  ...
  for (const code of codes) {
    console.error(`Fetching set metadata for ${code}...`);
    const metadata = await fetchSetMetadata(code);
    if (!metadata) {
      console.error(`Warning: set '${code}' not found on Scryfall, skipping.`);
      continue;
    }
    const raw = await fetchAllCardsForSet(code);
    const normalized = raw.map(normalizeCard);
    if (normalized.length < metadata.card_count) {
      const gap = metadata.card_count - normalized.length;
      console.error(`  Warning: ${gap} cards may still be unrevealed for ${code}.`);
    }
    perSet.push({
      set_code: code,
