...
# Pages-only deploy: rebuild SPA, then subtree-push Pages
npm run deploy:pages
```

`npm run deploy` expands to `deploy:api` then `deploy:pages`. `npm run deploy:pages` expands to `publish:hub` then `deploy:hub`. The final subtree push sends `rayenz-hub/` to the [rayenz-akusiom](https://github.com/rayenz-akusiom/rayenz-akusiom) repo `main` branch (GitHub Pages). Hub API details: [docs/hub-api-production.md](docs/hub-api-production.md).
