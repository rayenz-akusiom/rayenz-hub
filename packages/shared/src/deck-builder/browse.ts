 ...
 * New Set In / Out are omitted (shown only via the formal swap queue).
 */
export function partitionCategories(deck: Pick<DeckDocument, 'cards' | 'categories'>): {
  ...
  for (const [name, list] of Object.entries(groups)) {
    if (isSwapQueueCategory(name)) continue;
